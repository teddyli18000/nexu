...
/** One representative slug per category for the detail-page smoke checks. */
const SPOT_CHECK_SLUGS = [
  "email-management", // office-collab
  "document-search", // file-knowledge
  "image-generation", // creative-design
  "data-analysis", // biz-analysis
  "video-creation", // av-generation
  "web-research", // info-content
...
// ---------------------------------------------------------------------------
// Suite 5: Skill Browsability Smoke Test
// ---------------------------------------------------------------------------

test.describe("skill-browsability-smoke", () => {
  test.beforeEach(async ({ page }) => {
    await signIn(page);
  });

  test("GET /api/v1/skills returns exactly 63 official skills", async ({
    page,
  }) => {
    // Authenticated API call from within the browser session
    await page.goto(`${BASE_URL}/workspace`);
    ...
    const result = await page.evaluate(async (apiUrl) => {
      ...
      const body = (await res.json()) as {
        skills: Array<{ slug: string; source: string; tag: string }>;
    ...
    // Total official skills should be 63
    const officialSkills = result.body.skills.filter(
      (s) => s.source === "official",
    );
    expect(officialSkills).toHaveLength(63);

    // All 7 tags should be present
    expect(result.body.tags).toHaveLength(7);

    const tagIds = result.body.tags.map((t) => t.id);
    expect(tagIds).toContain("office-collab");
    expect(tagIds).toContain("file-knowledge");
    expect(tagIds).toContain("creative-design");
    expect(tagIds).toContain("biz-analysis");
    expect(tagIds).toContain("av-generation");
    expect(tagIds).toContain("info-content");
    expect(tagIds).toContain("dev-tools");
  });

  test("GET /api/v1/skills returns required fields for every skill", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/workspace`);
    ...
    for (const skill of result.skills) {
      expect(skill.slug.length, "slug missing on skill").toBeGreaterThan(0);
      expect(
        skill.name.length,
        `name missing on ${skill.slug}`,
      ).toBeGreaterThan(0);
      expect(
        skill.description.length,
        `description missing on ${skill.slug}`,
      ).toBeGreaterThan(0);
      expect(
        skill.iconName.length,
        `iconName missing on ${skill.slug}`,
      ).toBeGreaterThan(0);
      expect(
        skill.prompt.length,
        `prompt missing on ${skill.slug}`,
      ).toBeGreaterThan(0);
      expect(skill.tag.length, `tag missing on ${skill.slug}`).toBeGreaterThan(
        0,
      );
      expect(["official", "custom"]).toContain(skill.source);
    }
  });

  test("skills page renders all skill cards matching the API count", async ({
    page,
  }) => {
    // Get the expected count from the API
    await page.goto(`${BASE_URL}/workspace`);
    ...
    const apiResult = await page.evaluate(async (apiUrl) => {
      ...
      const body = (await res.json()) as { skills: Array<{ source: string }> };
      return body.skills.filter((s) => s.source === "official").length;
    }, API_URL);

    // Navigate to skills page and count rendered cards under Official tab
    await goToSkillsPage(page);

    // Ensure Official tab is active (default)
    await page.locator("button", { hasText: "Official" }).click();

    // Wait briefly for any re-render
    ...
    // The rendered card count should match the API's official skill count
    expect(renderedCards).toBe(apiResult);
  });

  test("spot-check: representative skill slugs exist in API response", async ({
    page,
  }) => {
    await page.goto(`${BASE_URL}/workspace`);

    const slugsInApi = await page.evaluate(async (apiUrl) => {
      const res = await fetch(`${apiUrl}/api/v1/skills`, {
        credentials: "include",
      });
      const body = (await res.json()) as { skills: Array<{ slug: string }> };
      return body.skills.map((s) => s.slug);
    }, API_URL);

    for (const slug of SPOT_CHECK_SLUGS) {
      expect(
        slugsInApi,
        `Expected slug "${slug}" to exist in /api/v1/skills response`,
      ).toContain(slug);
    }
  });

  test("spot-check: each representative skill slug page can be visited [placeholder]", async ({
    page,
  }) => {
    /**
     * When the /workspace/skills/:slug route is implemented, this test
     * will navigate to each representative slug and assert the page loads
     * without a 404 or error state.
     *
     * Currently marked fixme because the route does not exist yet.
     */
    test.fixme(
      true,
      "Skill detail route /workspace/skills/:slug not yet implemented",
    ...
    for (const slug of SPOT_CHECK_SLUGS) {
      ...
      // Should NOT redirect to / (which would happen for an unknown route)
      expect(page.url()).toContain(`/skills/${slug}`);

      // Should not show a generic error state
      await expect(
        page.locator("text=/404|not found|error/i"),
      ).not.toBeVisible();

      // The skill name should be rendered
      const heading = page.locator("h1, h2").first();
      await expect(heading).toBeVisible();
    }
  });

  test("header skill count on list page matches API total count", async ({
    page,
  }) => {
    // Get total skills from API (all sources)
    await page.goto(`${BASE_URL}/workspace`);

    const apiTotal = await page.evaluate(async (apiUrl) => {
      const res = await fetch(`${apiUrl}/api/v1/skills`, {
        credentials: "include",
      });
    ...
    // The header displays "{n} skills" where n = total count including custom
    const countLabel = page.locator(".sticky.top-0 >> text=/\\d+ skills/");
    ...
    expect(displayedCount).toBe(apiTotal);
  });
});
